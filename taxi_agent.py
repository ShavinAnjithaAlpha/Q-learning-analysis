...
def calculate_convergence_speed(convergence_values):
    ...
    window_size = 20
    convergence_values = np.convolve(convergence_values, np.ones(
    ...
    convergence_speed, intercept, _, _, _ = linregress(np.arange(
        len(convergence_values[:converged_index])), convergence_values[:converged_index])

    # plot the linear model
    if PLOT:
        plt.plot(np.arange(
            len(convergence_values[:converged_index])), convergence_values[:converged_index])
        plt.plot(np.arange(len(convergence_values[:converged_index])), intercept +
                 convergence_speed * np.arange(len(convergence_values[:converged_index])))
        plt.xlabel("Episode")
...
def get_mean_convergence_speed(state_size, action_size, env, learning_rate, discount_rate):
    ...
    if DEBUG:
        print(f"Convergence speed: {speed_mean:.5f}")
...
def generate_heat_map(state_size, action_size, env):
    ...
    for learning_rate in range(11):
        for discount_factor in range(11):
            if DEBUG:
                print(f"(x, y) = ({learning_rate}, {discount_factor})")
            # calculate the convergence speed for each learning rate and discount factor
            ret = get_mean_convergence_speed(
                state_size, action_size, env, learning_rate/10, discount_factor/10)
            if ret == -1:
                convergence_speed_data[learning_rate, discount_factor] = 0
                writer.writerow([learning_rate/10, discount_factor/10, 0])
            else:
                # increase the scale of the heatmap
                convergence_speed_data[learning_rate,
                                       discount_factor] = ret[0] * 100
...
def generate_convergence_data_fixed_discount(state_size, action_size, env, discount_rate, file_name):
    ...
    with open(file_name, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(
            ['Learning Rate', 'Convergence Speed', 'Standard Deviation'])
        learning_rate = 0
        while learning_rate <= 1:
            ret = get_mean_convergence_speed(
                state_size, action_size, env, learning_rate, discount_rate)
            if ret == -1:
            ...
            writer.writerow(
                [learning_rate, mean_convergence_speed, std_convergence_speed])
...
def generate_convergence_data_fixed_learning(state_size, action_size, env, learning_rate, file_name):
    ...
    with open(file_name, mode='w', newline='') as file:
        ...
        while discount_rate <= 1:
            ...
            mean_convergence_speed, std_convergence_speed = ret
            writer.writerow(
                [discount_rate, mean_convergence_speed, std_convergence_speed])
...
def main():
    ...
    if DATA_FILE:
        ...
        while discount_factor <= 1.0:
            ...
            generate_convergence_data_fixed_discount(
                state_size, action_size, env, discount_factor, file_name)
            print(
                f"data file generated for discount factor: {discount_factor}")
            discount_factor += 0.1
        ...
        while learning_rate <= 1.0:
            ...
            generate_convergence_data_fixed_learning(
                state_size, action_size, env, learning_rate, file_name)
